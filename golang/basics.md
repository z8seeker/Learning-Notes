...
整数类型有无符号和带符号两种。Go 同时支持 int 和 uint，这两种类型的长度相同。每一种数值类型都决定了其值的长度和是否有符号（size and signedness）。

Go 里面也有直接定义好位数的类型：`rune`, `int8`, `int16`, `int32`, `int64` 和 `byte`, `uint8`, `uint16`, `uint32`, `uint64`。其中 `rune` 是 `int32` 的别称，表明该类型的值是一个 Unicode 码点，`byte` 是 `uint8` 的别称, 表明该类型的值是一段二进制数据。还有两种类型:`int` 和 `uint`，它们对于在特定的平台定义的有符号或无符号数是最高效的长度，这两种类型的长度相同，为 32 位或 64 位。
